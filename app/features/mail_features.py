...
def mail_1(new_logs: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    описание фичи: частота отправки электронных писем
    """
    feature_name = 'mail_1'
...
def mail_2(new_logs: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    ...
    # условие
    new_logs = new_logs[new_logs.metadata.str.contains('"bcc": NaN') == False].reset_index(drop=True)
...
def mail_3(new_logs: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    описание фичи: частота логонов
    """
    feature_name = 'mail_3'
    ...
    # условие
    new_logs = new_logs[new_logs.metadata.str.contains('"attachments": 0') == False].reset_index(drop=True)
